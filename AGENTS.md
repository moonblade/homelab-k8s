...
- **DO NOT use `image:latest`** - always pin to a specific version (semver or timestamp tags). The `latest` tag causes unnecessary image pulls and wastes bandwidth on a metered connection.
- **ALWAYS set `imagePullPolicy: IfNotPresent`** - on all containers and initContainers in deployments, cronjobs, and any pod spec. Never omit this field.
...
- **Logging**: Loki + Promtail in `loki` namespace. Promtail DaemonSet ships all pod logs to Loki. Grafana (in `grafana` namespace) has Loki as default datasource.
- **Audiobookshelf duplicates**: Known ABS scanner bug ([#1447](https://github.com/advplyr/audiobookshelf/issues/1447), [PR #4621](https://github.com/advplyr/audiobookshelf/pull/4621)) creates duplicate DB entries for the same path during auto-scan. The watcher is disabled (NFS + inotify causes duplicates during file copy). Auto-scan runs every 2 hours (`0 */2 * * *`) and is the only scan mechanism. A dedup cronjob was tried and removed — it caused a delete→recreate loop and broke user progress/bookmarks. NFSv4.2 is in use with stable server-side inodes, so inode collisions are NOT the cause. Living with occasional duplicates until upstream fix is merged. DO NOT re-add a dedup cronjob — it makes things worse.
