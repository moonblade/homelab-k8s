...
- **DO NOT use `image:latest` tag** - always pin to a specific version (e.g. `v1.2.3`, `0.5.1`, `ts_*`). The `latest` tag causes unnecessary image pulls and wastes bandwidth on a metered connection.
- **DO NOT omit `imagePullPolicy`** - always set `imagePullPolicy: IfNotPresent` on all containers and initContainers. The default policy for pinned tags should be explicit to prevent unnecessary pulls. This applies to deployments, cronjobs, and any pod spec.
